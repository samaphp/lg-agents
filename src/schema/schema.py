...
class UserInput(BaseModel):
    ...
    message: str | None = Field(
        description="User input to the agent. Required for chat-based agents.",
        examples=["What is the weather in Tokyo?"],
        default=None,
    )
    state: dict[str, Any] | None = Field(
        description="Initial state for state-based agents like marketing_agent.",
        default=None,
        examples=[{"appName": "My Cool App", "max_personas": 3}],
    )
    ...
    def check_message_or_state(self) -> 'UserInput':
        if self.message is None and self.state is None:
